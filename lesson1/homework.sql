...
# 4
# (fast, 1 request)
select *, (price * product_count) as total from phones
where (price * product_count) between 100000 and 145000;
# (slow, 2 request - select all rows)
select * from (select *, (price * product_count) as total from phones) p
where total between 100000 and 145000;
# (fast, 2 request)
select p.*, @total as total from phones p, (select @total := 0) t
where id in (
    select id from phones
    where (@total := price * product_count) between 100000 and 145000
    );

select * from phones where phone_name like '%iphone%'; # 4.1
select * from phones where phone_name like '%galaxy%'; # 4.2
select * from phones where phone_name rlike '[0-9]+'; # 4.3
select * from phones where phone_name rlike '8+'; # 4.4