# 1. Создание БД
CREATE DATABASE IF NOT EXISTS gb_1; # Создать БД, если не существует
# 2. Подключение
USE gb_1; # Подключиться к БД
# 3. Создание таблицы
...
VALUES
	('admin', 'password_hash', 'admin@test.tt', ''),
	('test', 'password_hash', 'test@test.tt', '9997770001'),
    ('test2', 'password_hash', 'test2@test.tt', '9997770002');
# 5. Обновление данных
UPDATE users SET `password`='', `active`=false WHERE `username`='admin'; # Обновление записи в таблице
# 6. Выборка
